...
export default function CourseList() {
  const { store } = useContext(CourseByCategoryContext);
  ...
  return (
    <div>
      <div>
        <h2 className="mb-1 text-left">CATEGORY COURSES</h2>
      </div>
      <div className="row course-items-area">
      {store.courses.map((item) => (
          <div className="mix col-lg-3 col-md-4 col-sm-6 finance ">
              <a class=" hvr-shrink course-item" style={{width:"210px"}} onClick={()=>handleCourseItemRouting_Clicked(item.course_id)}>
                  <img
