...
export default function WebCourseList() {
  const { store } = useContext(webCourseContext);
  return (
    <div>
      <div>
        <h2 className="mb-1 text-left">WEB COURSES</h2>
      </div>
      <div className="row course-items-area">
      {store.webCourse.map((item) => (
          <div className="mix col-lg-3 col-md-4 col-sm-6 finance">
            <div className="course-item">
              ...
              >
                <div className="price">
                  Price:{" "}
                  {store.webCourse && store.webCourse.length !== 0
                    ? item.price
                    : 0}
                  &#65284;
                </div>
